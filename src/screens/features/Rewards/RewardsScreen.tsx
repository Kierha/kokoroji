...
import { grantReward } from "../../../services/rewardGrantService";
...
export default function RewardsScreen() {
    ...
    // Attribution reward (feedback)
    const [rewardGranting, setRewardGranting] = useState(false);
    const [grantResult, setGrantResult] = useState<null | { success: boolean; message: string }>(null);
        ...
        <SafeAreaView style={styles.safe} edges={["top"]}>
            ...
            <RewardSelectionModal
                ...
                onConfirm={async (selectedIds) => {
                    if (!familyId || !selectReward) return;
                    setRewardGranting(true);
                    try {
                        ...
                        await writeRewardLog({
                            ...
                            context: "Attribution de récompense réussie",
                        ...
                        // Rafraîchir liste enfants (solde mis à jour) + rewards si besoin futur
                        const famChildren = await getChildren(familyId);
                        const mapped = (famChildren ?? []).map((c: any) => ({
                            id: Number(c.id),
                            name: c.name,
                            avatarUri: c.avatar ?? null,
                            korocoins: Number.isFinite(Number(c.korocoins)) ? Number(c.korocoins) : 0,
                        ...
                        setGrantResult({ success: true, message: "Récompense attribuée avec succès." });
                    } catch (e: any) {
                        await writeRewardLog({
                            familyId,
                            logType: "error",
                            level: "error",
                            context: "Echec attribution récompense",
                            refId: selectReward.id,
                            details: { message: String(e?.message ?? e) },
                        });
                        setGrantResult({ success: false, message: String(e?.message || "Erreur d'attribution") });
                    } finally {
                        setRewardGranting(false);
                        setSelectVisible(false);
                        setSelectReward(null);
                    }
                }}
                loading={rewardGranting}
            />

            {/* Feedback attribution (succès / échec) */}
            <AppAlertModal
                visible={!!grantResult}
                title={grantResult?.success ? "Succès" : "Erreur"}
                message={grantResult?.message || ""}
                confirmLabel="OK"
                onConfirm={() => setGrantResult(null)}
            />
